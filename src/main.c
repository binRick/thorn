...
// Apply damage to enemy i with blood + death/victory handling (shared by all weapons).
static void HitEnemy(int i,int dmg){
    Enemy*e=&g_en[i]; e->hp-=dmg; e->hitFlash=0.12f; FloatDmg(e->x+EW*0.5f,e->y,dmg);
    if(e->hp<=0){ e->alive=0; e->st="DEAD"; PlayEnemyDeath(); Emit(e->x+EW*0.5f,e->y+EH*0.5f,16,300,0.6f,2.6f,(Color){170,30,30,255},0,1); g_shake=fmaxf(g_shake,7.0f); Ev("enemy %d killed",i); DebugLog("death","\"who\":\"enemy\",\"i\":%d,\"x\":%.1f,\"y\":%.1f",i,e->x,e->y); if(e->type==3){ g_victory=1; g_won=1; g_shake=20.0f; DebugLog("victory",""); Ev("THE USURPER FALLS"); } }
    else { SndPlay(SND_HIT); Emit(e->x+EW*0.5f,e->y+EH*0.5f,12,300,0.5f,2.3f,(Color){190,32,32,255},0,1); DebugLog("hit","\"who\":\"enemy\",\"i\":%d,\"dmg\":%d,\"hp\":%d",i,dmg,e->hp); }
}
// nearest live enemy whose centre is within +/-band of ray height ry, ahead in dir.
static int NearestInBand(float mx,float ry,int dir,float band,float range){
    int hit=-1; float best=range;
    for(int i=0;i<g_enN;i++){ Enemy*e=&g_en[i]; if(!e->alive||e->inCover) continue;
        float ex=e->x+EW*0.5f, ey=e->y+EH*0.5f, d=(ex-mx)*dir;
        if(d>0 && d<best && fabsf(ey-ry)<band && LineClear(mx,ex,gunY())){ best=d; hit=i; } }
    return hit;
}
...
static void BuildHero(int frame,int mode,int lean,int reach){
    ...
    } else if(mode>=3){              // OVER-THE-SHOULDER SWING: idx 0 forward .. 4 back, arcing over the top
        int idx=mode-3; if(idx>4)idx=4;
        float th=idx*(SPR_PI/4.0f);                         // 0 forward(+x), pi/2 up, pi back(-x)
        float dx=cosf(th), dy=-sinf(th); int sx=cx+4, sy=shY+2;
        int ex=(int)(sx+dx*8),  ey=(int)(sy+dy*8);
        int hx2=(int)(sx+dx*12),hy2=(int)(sy+dy*12);
        int gx=(int)(sx+dx*16), gy=(int)(sy+dy*16);
        limb(sx,sy,ex,ey,2,1);                              // upper arm
        limb(ex,ey,hx2,hy2,2,3);                            // bare forearm
        limb(hx2,hy2,gx,gy,2,5);                            // rifle along the aim line
        fillDisc(hx2,hy2,2,2,1);                            // hand
        if(idx>=3){ fillDisc((int)(sx+dx*19),(int)(sy+dy*19),3,3,9); }   // muzzle flash near the back
    } else {
...
static void InitSprites(void){
    ...
    g_sHeroFireBack=LoadStrip("assets/sprites/hero_fireback.png",5,16.0f); // over-the-shoulder swing
...
static int DumpSprites(void){
    ...
    a[n++]=HeroFrame(blue,0,5,0,0);      // over-the-shoulder swing (gun up)
...
static int GenAssets(void){
    ...
    { Image f[5]; for(int k=0;k<5;k++) f[k]=HeroFrame(blue,0,3+k,0,0); StripExport("assets/sprites/hero_fireback.png",f,5,NULL); for(int k=0;k<5;k++) UnloadImage(f[k]); }
...
static void DrawWorld(void){
    ...
    if(!P.dead){
        if(g_sprites){ float a=P.inCover?0.5f:(P.iframes>0?0.5f:1.0f); int moving=fabsf(P.vx)>5; Strip*s=NULL;
            ...
            if(s){ int fr=AnimF(s);
                if(s==&g_sHeroFireBack){ float pr=1.0f-P.muzzle/0.18f; if(pr<0)pr=0; if(pr>1)pr=1; float ph=pr<0.5f?pr*2.0f:(1.0f-pr)*2.0f; fr=(int)(ph*4.0f+0.5f); if(fr>4)fr=4; }
                DrawActorStrip(s,fr,P.x,P.y,PW,PH,P.face,a,0); }
...
#if !defined(__APPLE__) && !defined(__EMSCRIPTEN__)
        if(FileExists("assets/thorn_icon.png")){          // window icon (Linux/Windows; macOS has no window icons)
